<?php
/**
 * Plugin Name: DeVault Payment for Woocommerce
 * Plugin URI: https://devault.cc
 * Author Name: Pest
 * Author URI: https://desmadrecity.com
 * Description: This plugin allows for DeVault payments.
 * Version: 0.1.0
 * License: 0.1.0
 * License URL: http://www.gnu.org/licenses/gpl-2.0.txt
 * text-domain: dvtpay-woo
 *
 * Class WC_Gateway_DVTPay file.
 *
 * @package WooCommerce\DVTPay
 */
...
add_action( 'plugins_loaded', 'dvtpay_init', 11 );

function dvtpay_init() {
    if( class_exists( 'WC_Payment_Gateway' ) ) {
       /**
       * DVTPay Gateway.
       *
       * Provides a DeVault Payment Gateway.
       * @class       WC_Gateway_DVTPay
       * @extends     WC_Payment_Gateway
       * @version     0.1.0
       * @package     WooCommerce/Classes/Payment
       */

      class WC_Gateway_DVTPay extends WC_Payment_Gateway {
        ...
        protected function setup_properties() {
          ...
          $this->method_description = __( 'Have your customers pay with DeVault.', 'dvtpay-payments-woo' );
        ...
        public function init_form_fields() {
          $this->form_fields = array(
            ...
            'enable_for_methods' => array(
              'title'             => __( 'Enable for shipping methods', 'dvtpay-payments-woo' ),
              'type'              => 'multiselect',
              'class'             => 'wc-enhanced-select',
              'css'               => 'width: 400px;',
              'default'           => '',
              'description'       => __( 'If DVT is only available for certain methods, set it up here. Leave blank to enable for all methods.', 'dvtpay-payments-woo' ),
              'options'           => $this->load_shipping_method_options(),
              'desc_tip'          => true,
              'custom_attributes' => array(
                'data-placeholder' => __( 'Select shipping methods', 'dvtpay-payments-woo' ),
              ),
            ),
            'enable_for_virtual' => array(
              'title'   => __( 'Accept for virtual orders', 'dvtpay-payments-woo' ),
              'label'   => __( 'Accept DVT if the order is virtual', 'dvtpay-payments-woo' ),
              'type'    => 'checkbox',
              'default' => 'yes',
            ),
            'redirect_url' => array(
        ...
        private function is_accessing_settings() {
          if ( is_admin() ) {
            ...
            if ( ! isset( $_REQUEST['section'] ) || 'dvt' !== $_REQUEST['section'] ) {
              return false;
            }
        ...
        /**
         * Change payment complete order status to completed for DVT orders.
         *
         * @since  3.1.0
        ...
        public function change_payment_complete_order_status( $status, $order_id = 0, $order = false ) {
          if ( $order && 'dvt' === $order->get_payment_method() ) {
            $status = 'completed';
          }
          return $status;
        }
